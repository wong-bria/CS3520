...
print('More information will be written to %s' % LOG_FILE)
...
for suite in suites:
    ...
    for tt, case in enumerate(suite_cases):
        ...
        if build_result != 0:
            ...
            tested_returncode, tested_output = runcmd('tested implementation', tested_cmd, case_input)
            ref_returncode, ref_output = runcmd('reference implementation', ref_cmd, case_input)

            if tested_output == ref_output and tested_returncode == ref_returncode:
                num_passed += 1
